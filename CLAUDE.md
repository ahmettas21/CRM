...
*   **Architectural Integrity**: Masterplans and implementation plans MUST be cumulative. NEVER delete previously discussed/verified sections when focusing on a sub-task. Each update must be an `APPEND` or `ENHANCE` operation, not a total replacement that loses context.
